...
const RoleAssignCreate = ({ close , closeModal }) => {
  ...
  const fetchRoleAssign = async () => {
    ...
    try {
      const response1 = await axios.get(
        "public/api/admin/role/assign/employee/filter",
        {
      ...
      setEmployees(response1.data.filtered_employees);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "public/api/admin/role/assign/store",
        formData,
        { headers: { Authorization: `Bearer ${token}` } }
      );
        ...
        <div>
          <div className="col-span-3 bg-gray-200 p-6">
            <div className="max-w-7xl mx-auto md:px-8 flex justify-between items-center">
              <dt className="text-xl font-medium leading-6">NEW ROLE ASSIGN</dt>
              <button
                onClick={closeModal}
...
export default RoleAssignCreate;
