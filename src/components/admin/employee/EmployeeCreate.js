...
const EmployeeCreate = ({ close, closeModal }) => {
  ...
  const fetchEmployee = async () => {
    ...
    try {
      ...
      const response5 = await axios.get("public/api/admin/roles", {
        headers: { Authorization: `Bearer ${token}` },
      });
      setDesignation(response1.data.employee_designation);
      setCountry(response2.data.country);
      setState(response3.data.states);
      setDepartment(response4.data.employee_department);
      setRoles(response5.data.roles);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "public/api/admin/employees/store",
